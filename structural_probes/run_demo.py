...
def print_distance_image(args, words, prediction, sent_index):
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(args["reporting"]["root"], "demo-dist-pred" + str(sent_index)),
        dpi=300,
    )
...
def print_depth_image(args, words, prediction, sent_index):
    ...
    for index, (word, pred) in enumerate(zip(words, prediction)):
        plt.text(
            cumdist * 3, pred * 2, word, fontsize=fontsize, color="red", ha="center"
        )
    ...
    plt.savefig(
        os.path.join(args["reporting"]["root"], "demo-depth-pred" + str(sent_index)),
        dpi=300,
    )
...
def report_on_stdin(args):
    ...
    distance_probe.load_state_dict(
        torch.load(args["probe"]["distance_params_path"], map_location=args["device"])
    )

    # Define the depth probe
    depth_probe = probe.OneWordPSDProbe(args)
    depth_probe.load_state_dict(
        torch.load(args["probe"]["depth_params_path"], map_location=args["device"])
    )
    ...
    for index, line in tqdm(enumerate(sys.stdin), desc="[demoing]"):
        ...
        tokenized_sent = tokenizer.wordpiece_tokenizer.tokenize(
            "[CLS] " + " ".join(line.strip().split()) + " [SEP]"
        )
        untok_tok_mapping = data.SubwordDataset.match_tokenized_to_untokenized(
        ...
        with torch.no_grad():
                ...
                [
                    torch.mean(
                        single_layer_features[
                            0, untok_tok_mapping[i][0] : untok_tok_mapping[i][-1] + 1, :
                        ],
                        dim=0,
                    )
            ...
            distance_predictions = (
                distance_probe(representation.to(args["device"]))
                .detach()
                .cpu()[0][: len(untokenized_sent), : len(untokenized_sent)]
                .numpy()
            )
            depth_predictions = (
                depth_probe(representation)
                .detach()
                .cpu()[0][: len(untokenized_sent)]
                .numpy()
            )
            ...
            predicted_edges = reporter.prims_matrix_to_edges(
