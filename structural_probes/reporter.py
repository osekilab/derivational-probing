...
class Reporter:
    ...
    def __call__(self, prediction_batches, dataloader, split_name, state=None):
        """
        ...
        Args:
          ...
          state: some arbitrary state to report (written in json)
        """
        for method in self.reporting_methods:
            if method in self.reporting_method_dict:
                ...
                if method == "state":
                    self.reporting_method_dict[method](state, split_name)
                else:
    ...
    def report_state(self, state, split_name):
        with open(
            os.path.join(self.reporting_root, split_name + ".state"), "w"
        ) as fout:
