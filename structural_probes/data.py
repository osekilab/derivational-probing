...
class SimpleDataset:
    ...
    def read_from_disk(self):
        ...
        train_embeddings_path = os.path.join(
            self.args["dataset"]["embeddings"]["root"],
            self.args["dataset"]["embeddings"]["train_path"],
        )
        dev_embeddings_path = os.path.join(
            self.args["dataset"]["embeddings"]["root"],
        ...
        train_observations = self.optionally_add_embeddings(
            train_observations, train_embeddings_path
        )
        dev_observations = self.optionally_add_embeddings(
            dev_observations, dev_embeddings_path
        )
        test_observations = self.optionally_add_embeddings(
            test_observations, test_embeddings_path
        )
    ...
    def add_embeddings_to_observations(self, observations, embeddings):
        ...
        for observation, embedding in zip(observations, embeddings):
            embedded_observation = self.observation_class(
                *(observation[:-1]), embedding
            )
            embedded_observations.append(embedded_observation)
        return embedded_observations

    def generate_token_embeddings_from_hdf5(
        self, args, observations, filepath, layer_index
    ):
    ...
    def integerize_observations(self, observations):
        ...
        if self.vocab == {}:
            raise ValueError(
                "Cannot replace words with integer ids with an empty vocabulary "
    ...
    def custom_pad(self, batch_observations):
        ...
        if self.use_disk_embeddings:
            seqs = [
                torch.tensor(x[0].embeddings, device=self.args["device"])
                for x in batch_observations
            ]
        else:
            seqs = [
                torch.tensor(x[0].sentence, device=self.args["device"])
                for x in batch_observations
            ]
...
class ELMoDataset(SimpleDataset):
    ...
    def optionally_add_embeddings(self, observations, pretrained_embeddings_path):
        ...
        print(
            "Loading ELMo Pretrained Embeddings from {}; using layer {}".format(
                pretrained_embeddings_path, layer_index
            )
        )
        embeddings = self.generate_token_embeddings_from_hdf5(
            self.args, observations, pretrained_embeddings_path, layer_index
        )
...
class SubwordDataset(SimpleDataset):
    ...
    def generate_subword_embeddings_from_hdf5(
        self, observations, filepath, elmo_layer, subword_tokenizer=None
    ):
        raise NotImplementedError(
            "Instead of making a SubwordDataset, make one of the implementing classes"
        )
...
class BERTDataset(SubwordDataset):
    ...
    def generate_subword_embeddings_from_hdf5(
        ...
        if subword_tokenizer == None:
            try:
                ...
                if self.args["model"]["hidden_dim"] == 768:
                    ...
                    subword_tokenizer = BertTokenizer.from_pretrained(
                        "bert-large-cased"
                    )
                    print(
        ...
        single_layer_features_list = []
        for index in tqdm(
            sorted([int(x) for x in indices]), desc="[aligning embeddings]"
        ):
            observation = observations[index]
            feature_stack = hf[str(index)]
    ...
    def optionally_add_embeddings(self, observations, pretrained_embeddings_path):
        ...
        layer_index = self.args["model"]["model_layer"]
        print(
            "Loading BERT Pretrained Embeddings from {}; using layer {}".format(
                pretrained_embeddings_path, layer_index
            )
        )
...
class CumulativeDataset:
    ...
    def custom_pad(self, batch_observations):
        ...
        # seq: (num_layers, seq_len, d_model)
        lengths = torch.tensor([x.shape[1] for x in seqs], device=self.args["device"])
        seqs = torch.nn.utils.rnn.pad_sequence(
            [seq.transpose(0, 1) for seq in seqs], batch_first=True
        )
        seqs = seqs.transpose(
            1, 2
        )  # (batch_size, max_seq_len, num_layers, d_model) -> (batch_size, num_layers, max_seq_len, d_model)
        label_shape = batch_observations[0][1].shape
        maxlen = int(max(lengths))
        label_maxshape = [maxlen for x in label_shape]
        labels = [
        ...
        labels = torch.stack(labels)
        return seqs, labels, lengths, batch_observations
        # seqs: (b, n_layers, seq_len, d_model)
        # labels: (b, seq_len, seq_len)
        # lengths: (b,)
        # batch_observations: list of Observations


class BERTCumulativeDataset(CumulativeDataset):
    """Dataloader for conllx files and pre-computed BERT embeddings.
    ...
    def generate_subword_embeddings_from_hdf5(
        self, observations, filepath, layer_index, subword_tokenizer=None
    ):
        """Reads pre-computed subword embeddings from hdf5-formatted file.
        ...
        Args:
          ...
          observations: A list of Observations composing a dataset.
          filepath: The filepath of a hdf5 file containing embeddings.
          layer_index: The index corresponding to the layer, up to which the representation is used.
          subword_tokenizer: (optional) a tokenizer used to map from
              conllx tokens to subword tokens.
        ...
        hf = h5py.File(filepath, "r")
        indices = list(hf.keys())
        # cumlative layer features
        cumulative_features_list = []
        for index in tqdm(
            sorted([int(x) for x in indices]), desc="[aligning embeddings]"
        ):
            observation = observations[index]
            feature_stack = hf[
                str(index)
            ]  # (layer, subword_sent_length, feature_count)
            cumulative_features = feature_stack[: layer_index + 1]
            tokenized_sent = subword_tokenizer.wordpiece_tokenizer.tokenize(
                "[CLS] " + " ".join(observation.sentence) + " [SEP]"
            )
            untokenized_sent = observation.sentence
            ...
            cumulative_features = torch.stack(
                [
                    torch.tensor(
                        np.mean(
                            cumulative_features[
                ...
                0, 1
            )  # Transpose to get (layer, token, feature)
            assert cumulative_features.shape[0] == layer_index + 1
            assert cumulative_features.shape[1] == len(observation.sentence)
            cumulative_features_list.append(cumulative_features)
        return cumulative_features_list
...
class GPT2CumulativeDataset(CumulativeDataset):
    """Dataloader for conllx files and pre-computed BERT embeddings.
    ...
    def match_tokenized_to_untokenized(tokenized_sent, untokenized_sent):
        ...
        while untokenized_sent_index < len(
            ...
            while tokenized_sent_index < len(tokenized_sent) and not tokenized_sent[
                tokenized_sent_index
            ].startswith("Ġ"):
                mapping[untokenized_sent_index].append(tokenized_sent_index)
                tokenized_sent_index += 1
    ...
    def generate_subword_embeddings_from_hdf5(
        ...
        if subword_tokenizer == None:
            try:
                ...
                if self.args["model"]["hidden_dim"] == 768:
                    ...
                    print("Using gpt2 tokenizer to align embeddings with PTB tokens")
                elif self.args["model"]["hidden_dim"] == 1024:
                    subword_tokenizer = GPT2Tokenizer.from_pretrained("gpt2-medium")
                    print(
                        "Using gpt2-medium tokenizer to align embeddings with PTB tokens"
                    )
                else:
                    print("The heuristic used to choose BERT tokenizers has failed...")
                    exit()
        ...
        for index in tqdm(
            ...
            cumulative_features = feature_stack[: layer_index + 1]
            untokenized_sent = observation.sentence
            tokenized_sent = subword_tokenizer.tokenize(" ".join(untokenized_sent))
            untok_tok_mapping = self.match_tokenized_to_untokenized(
                tokenized_sent, untokenized_sent
            )
            assert cumulative_features.shape[1] == len(tokenized_sent)
            cumulative_features = torch.stack(
                [
                    torch.tensor(
                        np.mean(
    ...
    def optionally_add_embeddings(self, observations, pretrained_embeddings_path):
        ...
        layer_index = self.args["model"]["model_layer"]
        print(
            "Loading GPT2 Pretrained Embeddings from {}; using layer {}".format(
                pretrained_embeddings_path, layer_index
            )
        )
        embeddings = self.generate_subword_embeddings_from_hdf5(
...
class Llama3CumulativeDataset(CumulativeDataset):
    """Dataloader for conllx files and pre-computed BERT embeddings.
    ...
    def generate_subword_embeddings_from_hdf5(
        ...
        Returns:
        A list of torch tensors; one for each observation.

        Raises:
        AssertionError: sent_length of embedding was not the length of the
            corresponding sentence in the dataset.
        Exit: importing transformers has failed, possibly due
            to downloading of prespecified tokenizer problem. Not recoverable;
            exits immediately.
        """
        try:
            ...
            print("Couldn't import or initialize the tokenizer. Exiting...")
            exit()

        cumulative_features_list = []
        hf = h5py.File(filepath, "r")
        try:
            indices = sorted([int(x) for x in hf.keys()])
            for index in tqdm(indices, desc="[aligning embeddings]"):
                observation = observations[index]
                # Load and immediately process the data
                feature_stack = hf[str(index)][: layer_index + 1]
                    ...
                    [
                        torch.tensor(
                            np.mean(
                                feature_stack[
                                    :,  # All layers
                                    untok_tok_mapping[i][0] : untok_tok_mapping[i][-1]
                                    + 1,
                                    :,
                                ],
                                axis=1,  # Average over subwords
                ...
                # Explicitly close dataset objects
                hf[str(index)].id.close()
    ...
    def optionally_add_embeddings(self, observations, pretrained_embeddings_path):
        ...
        layer_index = self.args["model"]["model_layer"]
        print(
            "Loading LLama3 Pretrained Embeddings from {}; using layer {}".format(
                pretrained_embeddings_path, layer_index
            )
        )
        embeddings = self.generate_subword_embeddings_from_hdf5(
